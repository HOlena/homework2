...
namespace VariablesDemo
{
    class Program
    {
        static void Main(string[] args)
        {
            ...
            byte languageValue;
            byte.TryParse(line, out languageValue);

            if (languageValue > Convert.ToByte(Enum.GetNames(typeof(Language)).Length))
            {
                Console.WriteLine("To mach");
            }
             else
            if (languageValue <= Convert.ToByte(Enum.GetNames(typeof(Language)).Length) && languageValue > Convert.ToByte(0))
            {
